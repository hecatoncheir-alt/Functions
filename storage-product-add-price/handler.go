...
type Request struct{ DatabaseGateway, ProductID, PriceID string }
...
func Handle(req []byte) string {
	...
	err = executor.AddPriceToProduct(request.ProductID, request.PriceID)
	if err != nil {
		warning := fmt.Sprintf(
			"Add price to product error: %v", err)
