...
type Request struct{ DatabaseGateway, InstructionID, CityID string }
...
func Handle(req []byte) string {
	...
	err = executor.AddCityToInstruction(request.InstructionID, request.CityID)
	if err != nil {
		warning := fmt.Sprintf(
			"Add City to Instruction error: %v", err)
